(function($) {
    ...
    $.logiform = function(element, options) {
        ...
        logiform.init = function() {
            ...
            for (var i = 0, sz = logiform.settings.schema.length; i < sz; i++) {
                ...
                if (item.id == '|') {
                    ...
                    if (item['type'] == 'array') {
                        ...
                        fieldValueMockup[item['id']] = 
                            '<input '+(logiform.settings.readonly?'readonly ':'')+'class="lf-value form-control logiform" type="text" style="width:' +
                            logiform.settings.width.value+'">';
                    }
            ...
            var fieldContent = 
                '<select '+(logiform.settings.readonly?'disabled ':'')+'class="lf-field selectpicker logiform" data-width="'+logiform.settings.width.field+'">' +
                fieldItems +
                '</select>';
